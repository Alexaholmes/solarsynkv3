...
if BearerToken:       
    for serialitem in inverterserials:
        ...
        if varContest == "Connection Success":
            ...
            url = f"{SUPERVISOR_URL}/core/api/states/input_text.solarsynkv3_{serialitem}_settings_AH"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(ConsoleColor.FAIL + f"Error: Unable to connect to Home Assistant Settings via the API HTTP Error: {response.status_code}. Settings will not be processed or applied. Please create a text entity manually named(AH): [solarsynkv3_{serialitem}_settings]" + ConsoleColor.ENDC)
                    SettingsExist = False
